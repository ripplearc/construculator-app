...
class AddCostEstimationUseCase {
  ...
  Future<Either<Failure, CostEstimate>> call({
    ...
    try {
      _logger.debug('Creating cost estimation: $estimationName for project: $projectId');
      
      final now = _clock.now();
      
      // Create default markup configuration
      ...
      // Create the cost estimation with defaults
      ...
      _logger.debug('Created cost estimation entity');
      
      // Save the estimation through the repository
      final createdEstimation = await _repository.createEstimation(costEstimation);
      
      _logger.debug('Successfully created cost estimation: ${createdEstimation.id}');
      
      return Right(createdEstimation);
    } on ServerException {
      _logger.error('Error creating cost estimation: ServerException');
      return Left(ServerFailure());
    } on ClientException {
      _logger.error('Error creating cost estimation: ClientException');
      return Left(ClientFailure());
    } on NetworkException {
      _logger.error('Error creating cost estimation: NetworkException');
      return Left(NetworkFailure());
    } on TimeoutException {
      _logger.error('Error creating cost estimation: TimeoutException');
      return Left(NetworkFailure());
    } on TypeError {
      _logger.error('Error creating cost estimation: TypeError');
      return Left(ClientFailure());
    } catch (e) {
