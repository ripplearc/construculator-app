...
class AddCostEstimationBloc extends Bloc<AddCostEstimationEvent, AddCostEstimationState> {
  ...
  AddCostEstimationBloc({
    ...
    on<AddCostEstimationStarted>(_onStarted);
    on<AddCostEstimationSubmitted>(_onSubmitted);
    on<AddCostEstimationRetried>(_onRetried);
  }

  Future<void> _onStarted(
    AddCostEstimationStarted event,
    Emitter<AddCostEstimationState> emit,
  ) async {
    emit(const AddCostEstimationInitial());
  }
  ...
  Future<void> _onSubmitted(
    ...
    result.fold(
      (failure) {
        emit(const AddCostEstimationFailure(
  ...
  Future<void> _onRetried(
    AddCostEstimationRetried event,
    Emitter<AddCostEstimationState> emit,
  ) async {
    emit(const AddCostEstimationInProgress());
