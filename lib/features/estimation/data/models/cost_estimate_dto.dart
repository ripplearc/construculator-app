...
class CostEstimateDto {
    ...
    return CostEstimate(
      ...
      markupConfiguration: MarkupConfiguration(
        ...
        materialValue: materialMarkupValueType != null && 
            ...
            ? MarkupValue(
                type: _mapMarkupValueType(materialMarkupValueType as String),
                value: materialMarkupValue as double,
              )
            : null,
        laborValue: laborMarkupValueType != null && 
                    laborMarkupValue != null
            ? MarkupValue(
                type: _mapMarkupValueType(laborMarkupValueType as String),
                value: laborMarkupValue as double,
              )
            : null,
        equipmentValue: equipmentMarkupValueType != null && 
                        equipmentMarkupValue != null
            ? MarkupValue(
                type: _mapMarkupValueType(equipmentMarkupValueType as String),
                value: equipmentMarkupValue as double,
              )
            : null,
      ),
      totalCost: totalCost ?? 0,
      lockStatus: isLocked
          ? LockStatus.locked(lockedByUserID ?? '', DateTime.parse(lockedAt as String))
  ...
  MarkupType _mapMarkupType(String raw) {
    if (raw.isEmpty) {
      return MarkupType.overall; // Default to overall when empty
    }
  ...
  MarkupValueType _mapMarkupValueType(String raw) {
    if (raw.isEmpty) {
      return MarkupValueType.percentage; // Default to percentage when empty
    }
