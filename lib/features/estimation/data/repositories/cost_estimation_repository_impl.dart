...
class CostEstimationRepositoryImpl implements CostEstimationRepository {
  ...
  String _buildStreamKey(String projectId, EstimationSortOption sortBy, int? limit) {
  ...
  CostEstimate? _getOriginalEstimation(String projectId, String estimationId) {
    final projectKeys = _cachedEstimations.keys.where((k) => k.startsWith('$projectId:'));
  ...
  void _emitOptimisticUpdate({
    ...
    final projectKeys = _streamControllers.keys.where((k) => k.startsWith('$projectId:'));
    for (final key in projectKeys) {
  ...
  void _updateStreamWithNewEstimation(
    ...
    for (final key in projectKeys) {
      ...
      _logger.debug(
        'Updating stream with new estimation for stream: $key',
      );
