...
void main() {
  group('GlobalSearchBloc', () {
    ...
    test('initial state is GlobalSearchInitial with empty recentSearches and empty query', () {
      final bloc = Modular.get<GlobalSearchBloc>();
      expect(bloc.state, const GlobalSearchInitial());
      expect(bloc.state, isA<GlobalSearchInitial>().having(
        (s) => s.recentSearches,
        'recentSearches',
        isEmpty,
      ));
    ...
    group('GlobalSearchPerformed', () {
        ...
        setUp: () {
          fakeSupabase.setRpcResponse(
            DatabaseConstants.globalSearchRpcFunction,
            {
              ...
              'estimations': [
                estimation_factory.EstimationTestDataMapFactory
                    .createFakeEstimationData(id: 'e1'),
              ],
      ...
      blocTest<GlobalSearchBloc, GlobalSearchState>(
        'uses supplied scope — saves search term under estimation scope',
        setUp: () {
          fakeSupabase.setCurrentUser(
            FakeUser(
              id: _testUserId,
        ...
        verify: (_) async {
          ...
          expect(
            data[DatabaseConstants.scopeColumn],
    ...
    // -----------------------------------------------------------------------
    // GlobalSearchSuggestionsRequested
    // -----------------------------------------------------------------------
