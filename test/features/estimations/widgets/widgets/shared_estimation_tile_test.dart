...
void main() {
  ...
  group('SharedEstimationTile', () {
    group('Basic Rendering', () {
      ...
      testWidgets('displays estimate name', (tester) async {
        await tester.pumpWidget(
          createWidget(
            data: _FakeData(
              estimateName: 'My Custom Estimate',
    ...
    group('displayDate', () {
      ...
      testWidgets('different displayDate values produce different output', (
        tester,
      ) async {
    ...
    group('Menu semantics', () {
      testWidgets(
        'includes semantics label on menu button when onMenuTap is provided',
        (tester) async {
          await tester.pumpWidget(
            createWidget(
              data: _FakeData(
      ...
      testWidgets('excludes menu area from a11y tree when onMenuTap is null', (
        tester,
      ) async {
        await tester.pumpWidget(
          createWidget(
    ...
    group('Card semantics', () {
      testWidgets('card has a Semantics widget with button:true', (
        tester,
      ) async {
        await tester.pumpWidget(
          createWidget(
            data: _FakeData(
              estimateName: 'Bridge Project',
              displayDate: testDate,
            ),
          ),
        );
        ...
        final cardSemantics = semanticsWidgets.where(
          (s) =>
              s.properties.label == 'Bridge Project' &&
    ...
    group('Tap Callbacks', () {
      ...
      testWidgets(
        'does not crash when onMenuTap is null and menu icon tapped',
        (tester) async {
          await tester.pumpWidget(
            createWidget(
              data: _FakeData(estimateName: 'Test', displayDate: testDate),
    ...
    group('Edge Cases', () {
      testWidgets('handles very long estimate names without overflow errors', (
        tester,
      ) async {
        const longName =
