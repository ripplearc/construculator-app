...
void main() {
  group('CostEstimationLogBloc', () {
    ...
    const skipOneFetchInitialCycle = 2; // Loading + first Loaded.
    const skipTwoFetchInitialCycles =
        2 * skipOneFetchInitialCycle; // (Loading + Loaded) x2.
    ...
    setUp(() {
      ...
      // Repository caches pagination cursor; dispose to avoid test state bleed.
      repository.dispose();
      bloc = Modular.get<CostEstimationLogBloc>();
    });
    ...
    group('CostEstimationLogLoadMore', () {
      ...
      blocTest<CostEstimationLogBloc, CostEstimationLogState>(
        ...
        skip: skipTwoFetchInitialCycles,
        expect: () => [],
        verify: (bloc) {
          expect(
            bloc.state,
