...
void main() {
 group('FakeSupabaseWrapper', () {
  ...
  group('FakeSupabaseWrapper Authentication State', () {
    ...
    test(
      'simulateAuthStreamError emits an error on onAuthStateChange stream',
      () async {
...
>>>>>>> f0238ef (Group tests)
    ...
    test(
      'reset reverts all fake configurations, clears data, and auth state',
      () async {
        await fakeWrapper.signInWithPassword(
          email: 'user@example.com',
          password: 'password',
        ...
        expect(
          fakeWrapper.currentUser,
          isNull,
          reason: "Current user should be null after reset",
        );
        expect(
          fakeWrapper.isAuthenticated,
          isFalse,
          reason: "Should not be authenticated after reset",
        );
        expect(
          fakeWrapper.shouldThrowOnSignIn,
          isFalse,
          reason: "shouldThrowOnSignIn flag should be reset",
        );
        expect(
          fakeWrapper.signInErrorMessage,
          isNull,
          reason: "signInErrorMessage should be reset",
        );
        expect(
          fakeWrapper.shouldThrowOnSelect,
          isFalse,
          reason: "shouldThrowOnSelect flag should be reset",
        );
        expect(
          fakeWrapper.selectErrorMessage,
          isNull,
          reason: "selectErrorMessage should be reset",
        );
        ...
        expect(
          userResult,
          isNull,
          reason: "User data should be cleared by reset",
        );
      },
    );
  });
