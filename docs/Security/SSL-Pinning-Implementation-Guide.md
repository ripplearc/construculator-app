...
- A TLS certificate for your server (self-signed for dev, CA-issued for QA/prod)
...
Never disable pinning silently. Use your existing `Config` abstraction so pinning state is always intentional.
...
class NetworkFactory {
  ...
  static Dio create({
    ...
    return switch (config.environment) {
      ...
      Environment.dev => _unpinnedDio(baseUrl: baseUrl),

      // QA & Production: pinning always active
      Environment.qa ||
      Environment.prod =>
...
With pinning active in QA, your proxy tool should **fail** to intercept traffic — that is the intended behavior and the proof pinning works.

- Use `Environment.dev` for local debugging — proxy tools work freely there
- If a proxy succeeds in QA, pinning is not active — investigate the adapter setup
...
| Disable pinning only via `Config.environment` | Prevents accidental deploy with pinning off |
...
| Works on dev, fails on QA | Dev uses the unpinned client by design — check that `Config.environment` is set correctly |
| Pinning broke after cert renewal | New cert has a new public key — was the new SPKI hash added as `backup` before rotating? |
| Proxy interception succeeds on QA | Pinning is not active — verify `IOHttpClientAdapter` is wired up and env is not `Environment.dev` |
